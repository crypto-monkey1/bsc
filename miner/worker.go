...
const (
	...
	minRecommitInterval = 1 * time.Millisecond
...
func (w *worker) mainLoop() {
	...
	for {
		...
		case req := <-w.newWorkCh:
			if w.chain.CurrentBlock().Number().Cmp(w.prevBlockNumber) == 1 {
				w.prevBlockNumber = w.chain.CurrentBlock().Number()
