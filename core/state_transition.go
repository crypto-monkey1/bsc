...
func ApplyMessageForSimulator(evm *vm.EVM, msg Message, gp *GasPool, blockNumber *big.Int) (*ExecutionResult, error) {
	// log.Info("At ApplyMessage")
	return NewStateTransition(evm, msg, gp).TransitionDbForSimulator(blockNumber)
}
...
func (st *StateTransition) TransitionDbForSimulator(blockNumber *big.Int) (*ExecutionResult, error) {
	// First check this message satisfies all consensus rules before
	// applying the message. The rules include these clauses
	//
	// 1. the nonce of the message caller is correct
	...
	if contractCreation {
		...
		ret, st.gas, vmerr = st.evm.CallForSimulator(sender, st.to(), st.data, st.gas, st.value, blockNumber)
