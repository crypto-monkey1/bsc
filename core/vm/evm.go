...
func runForSimulator(evm *EVM, contract *Contract, input []byte, readOnly bool, blockNumber *big.Int) ([]byte, error) {
	for _, interpreter := range evm.interpreters {
		if interpreter.CanRun(contract.Code) {
			// evm.Context.BlockNumber = blockNumberToSimBigInt
			...
			return interpreter.RunForSimulator(contract, input, readOnly, blockNumber)
		}
	}
	return nil, errors.New("no compatible interpreter")
}
...
func (evm *EVM) CallForSimulator(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int, blockNumber *big.Int) (ret []byte, leftOverGas uint64, err error) {
	if evm.vmConfig.NoRecursion && evm.depth > 0 {
		return nil, gas, nil
	}
	// Fail if we're trying to execute above the call depth limit
	...
	if isPrecompile {
		...
		if len(code) == 0 {
			...
			ret, err = runForSimulator(evm, contract, input, false, blockNumber)
			gas = contract.Gas
		}
	}
