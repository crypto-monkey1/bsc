...
type Interpreter interface {
	...
	RunForSimulator(contract *Contract, input []byte, static bool, blockNumber *big.Int) ([]byte, error)
...
func (in *EVMInterpreter) RunForSimulator(contract *Contract, input []byte, readOnly bool, blockNumber *big.Int) (ret []byte, err error) {
	...
	for {
		...
		originalBlockNumber := in.evm.Context.BlockNumber
		if op == NUMBER {
			in.evm.Context.BlockNumber = blockNumber
		}

		if op == BLOCKHASH {
			res, err = opBlockhashCustom(&pc, in, callContext, blockNumber)
		} else {
			// execute the operation
			res, err = operation.execute(&pc, in, callContext)
		}
		...
		if op == NUMBER {
			in.evm.Context.BlockNumber = originalBlockNumber
		}
