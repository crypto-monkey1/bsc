...
func applyTransactionCustom(msg types.Message, config *params.ChainConfig, bc ChainContext, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, evm *vm.EVM, blockNumberToSimBigInt *big.Int, timestampOverride uint64, receiptProcessors ...ReceiptProcessor) (*types.Receipt, error) {
	// Create a new context to be used in the EVM environment.
	// log.Info("At applyTransactionCostumHeader")
	txContext := NewEVMTxContext(msg)
	evm.Reset(txContext, statedb)
...
func applyTransactionForSimulator(msg types.Message, config *params.ChainConfig, bc ChainContext, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, evm *vm.EVM, blockNumber *big.Int, receiptProcessors ...ReceiptProcessor) (*types.Receipt, error) {
	// Create a new context to be used in the EVM environment.
	// log.Info("At applyTransactionCostumHeader")
	txContext := NewEVMTxContext(msg)
	evm.Reset(txContext, statedb)
	...
	// Apply the transaction to the current state (included in the env).
	result, err := ApplyMessageForSimulator(evm, msg, gp, blockNumber)
	if err != nil {
		return nil, err
	}
	// log.Info("After ApplyMessage")
...
func ApplyTransactionCostumHeader(config *params.ChainConfig, bc ChainContext, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, cfg vm.Config, blockNumberToSimBigInt *big.Int, timestampOverride uint64, receiptProcessors ...ReceiptProcessor) (*types.Receipt, error) {
	// log.Info("At ApplyTransactionCostumHeader")
	// headerCostum := header
