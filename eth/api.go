...
func (api *PrivateMinerAPI) SimulateOnCurrentStatePriority(addressesToReturnBalances []common.Address, addressesToDeleteFromPending []common.Address, blockNumberToSimulate *big.Int, inputPriority hexutil.Bytes, inputTxs []hexutil.Bytes, stoppingHash common.Hash, returnedDataHash common.Hash) map[string]interface{} {
	txsArray := make([]types.Transaction, len(inputTxs))
	for i, input := range inputTxs {
		if err := txsArray[i].UnmarshalBinary(input); err != nil {
	...
	priorityTx := &types.Transaction{}
	if err := priorityTx.UnmarshalBinary(inputPriority); err != nil {
		log.Error("Simulator: Couldnt unmarshal priority tx", "err", err)
		return nil
	}
	return api.e.SimulateOnCurrentStatePriority(addressesToReturnBalances, addressesToDeleteFromPending, blockNumberToSimulate, priorityTx, txsArray, stoppingHash, returnedDataHash)
}
...
func (api *PrivateMinerAPI) SimulateNextTwoStates(addressesToReturnBalances []common.Address, addressesToDeleteFromPending []common.Address, x1BlockNumber *big.Int, inputPriorityX2 hexutil.Bytes, x2InputTxs []hexutil.Bytes, x3InputTxs []hexutil.Bytes, stoppingHash common.Hash, stopReceiptHash common.Hash, returnedDataHash common.Hash) map[string]interface{} {
	...
	priorityX2Tx := &types.Transaction{}
	if err := priorityX2Tx.UnmarshalBinary(inputPriorityX2); err != nil {
		log.Error("Simulator: Couldnt unmarshal priority tx", "err", err)
		return nil
	}
	return api.e.SimulateNextTwoStates(addressesToReturnBalances, addressesToDeleteFromPending, x1BlockNumber, priorityX2Tx, x2TxsArray, x3TxsArray, stoppingHash, stopReceiptHash, returnedDataHash)
}
